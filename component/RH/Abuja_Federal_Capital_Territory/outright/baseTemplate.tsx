...
export const BaseOutrightLetter = (props: props) => {
  const totalMonths =
    ...
    <div className="">
      ...
      <div className="h-[297mm] pt-10 p-6">
        <ol start={6} className="list-decimal text-sm px-8 space-y-4">
          <li>
            <span className="font-semibold">FIRST INSTALLMENT:</span> &#8358;
          ...
          <li>
            ...
            of sublease/survey plan shall b e issued to the Purchaser. The
          ...
          <li>
            ...
            <NumberToWordsConverter number={balance} currency="Naira" /> shall
            be paid within{" "}
            {installments === 1
              ? "1 (one) month,"
              : `${totalMonths} Months under the following terms`}
          </li>
          <ol className="px-4 mt-2 list-[lower-alpha] space-y-2" type="A">
            {installments > 1 && (
              <>
                {milestones.map((m) => (
                  <li key={m.number}>
                    Milestone {m.number}: payment of &#8358;{" "}
                    {formatToCurrencyString(m.amount)}{" "}
                    <NumberToWordsConverter
                      number={m.amount}
                      currency="Naira"
                    />{" "}
                    to be paid on or before{" "}
                    {moment(futureDates[m.dueDateKey]).format("LL")}.
                  </li>
                ))}
              </>
            )}
        ...
        <p className="text-sm px-8 mt-4">
          ...
          All subscribers covenant t o abide b y the terms and conditions
          contained in the final offer letter and the Contract of Sale, in
          addition t o all conditions and terms imposed b y the Federal Capital
          Authority relating t o payments of rates and taxes and such other
          statutory charges
          <br />
          Kindly indicate your acceptance o f this offer by signing in the space
          provided be herein and return the duly signed copy o f this
          Provisional offer.
        </p>
